...
* 	successivi N*M*8 byte: matrix data in row-major order --> numeri typeing-point a precisione doppia
* 
*****************************************************************************
...
* 	successivi N*M*8 byte: matrix data in row-major order --> numeri interi o typeing-point a precisione doppia
*/
...
* 	successivi byte: elementi del vettore 		--> 1 numero typeing-point a precisione doppia e k interi
*/
...
void cfs(params* input){
	...
	for(int fi=0; fi<input->d; fi++)
		mediaP4(fi, input->N%4, mu, input, strcf);

	int fmax = rcfP4(input->d%2, input->N%4, mu, input, strcf);
	...
	type midrcf, midrff;
	type midrcfMax, midrffMax;
	type merit, meritMax;
	int fx=0, y=0;
	while(cardS<input->k) {
		meritMax=0.0;
		for(fx=0; fx<input->d; fx++) {
			if(inS[fx]==0){
				midrcf = midrcfIn + strcf[fx];
				midrff = midrffIn;

				for (y = 0; y < cardS; y++)
					midrff += rffP4(fx, input->out[y], input->N%4, mu, input);
...
int main(int argc, char** argv) {
	...
	input->ds = load_data_column(dsfilename, &input->N, &input->d);
